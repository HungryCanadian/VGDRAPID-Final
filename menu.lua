local Object = require "classic"  -- Import the classic OOP library
local Menu = Object:extend()  -- Create a new class for the menu

function Menu:new()
    self.options = {  -- Define the menu options
        {text = "Start Game", action = 
        function() print("Starting Game...") 
         self.playing = true end},
        {text = "Exit", action = function() love.event.quit() end},
    }
    self.font = love.graphics.newFont(24)  -- Set a larger font for visibility
    self.buttonWidth = 200
    self.buttonHeight = 50
    self.buttonSpacing = 20
    self.startY = 400  -- Starting Y position for buttons
    ...
    if button == 1 then  -- Left mouse button
        for i, _ in ipairs(self.options) do
            if self:isMouseOverButton(x, y, i) then
                self.options[i].action()  -- Trigger the corresponding action
            end
        end
    end
end
...
function Menu:draw()
    ...
    for i, item in ipairs(self.options) do
        ...
        if self:isMouseOverButton(mx, my, i) then
            love.graphics.setColor(1, 1, 0) -- Yellow for hover
        else
